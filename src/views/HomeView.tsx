...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  const [selectedGalleryImage, setSelectedGalleryImage] = useState<null | (typeof GALLERY_MEDIA)[0]>(null);
  ...
  return (
    <div id="homepage-container" className="w-full">
      {/* ========================================================================= */}
      {/* 1. HERO SECTION: Slideshow background with content overlay */}
      {/* ========================================================================= */}
      <section
        id="hero-section"
      ...
      <section id="gallery-preview-section" className="py-16 sm:py-24 bg-[#FFF9EC] relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {GALLERY_MEDIA.filter(m => m.type === 'photo').slice(0, 6).map((item) => (
              <div
                key={item.id}
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#173B5E]/90 via-[#173B5E]/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-5 text-white">
                  ...
                  <p className="text-xs text-white/80 line-clamp-1 mt-0.5">{item.subcategory}</p>
                </div>
              </div>
            ))}
          </div>
